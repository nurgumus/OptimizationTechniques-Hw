function [xk, fk, gk_norm, k_iter, path_hist] = polak_ribiere_optimizer(f, grad_f, x0, epsilon, max_iter, n_dim)
    xk = x0;
    fk = f(xk);
    gk = grad_f(xk);
    dk = -gk;
    ...
    for k_iter = 1:max_iter
        ...
        if alpha_k < 1e-8, warning('P-R: Line search step too small.'); break; end
        ...
        yk = gk_new - gk;
        denominator_pr_fr = gk' * gk;
        if abs(denominator_pr_fr) < 1e-10
             beta_k_plus_1 = 0; % Restart if denominator is too small
        else
            beta_k_plus_1 = (gk_new' * yk) / denominator_pr_fr;
        end
        beta_k_plus_1 = max(0, beta_k_plus_1); % Important for Polak-Ribiere+
    ...
    warning('Polak-Ribiere: Maximum iterations reached.');
end